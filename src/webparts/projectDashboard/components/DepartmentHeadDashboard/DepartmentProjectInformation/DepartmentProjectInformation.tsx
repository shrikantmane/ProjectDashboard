...
import { IDepartmentProjectProps } from "./IDepartmentProjectProps";
import { IDepartmentProjectState } from "./IDepartmentProjectState";
...
import {
  ...
  Tags,
  Documents
} from "../DepartmentDashboard";;
import { Link, Redirect } from 'react-router-dom';
import TimeLineChart from "../TimeLineChart/TimeLineChart";
import { ProjectTimeLine, Groups, TimeLineItems } from "../TimeLineChart/TimeLine";
import { find, filter, sortBy } from "lodash";
import moment from 'moment/src/moment';

export default class DepartmentProjectInformation extends React.Component<
  IDepartmentProjectProps,
  IDepartmentProjectState
  > {
  constructor(props) {
    super(props);
    this.state = {
      projectList: new Array<Projects>(),
  ...
  private projectNameTemplate(rowData: Projects, column) {
    return (
      <div>
  ...
  private ownerTemplate(rowData: Projects, column) {
  ...
  private mildstoneTemplate(rowData: Projects, column) {
    let startDate: any;
    let dueDate: any;
  ...
  private statusTemplate(rowData: Projects, column) {
    if (rowData.Status0)
      return (
  ...
  private priorityTemplate(rowData: Projects, column) {
    return <div className={styles.priorityDetail}>{rowData.Priority}</div>;
  }

  private rowExpansionTemplate(data: Projects) {
  ...
  private getProjectTimeLineData(projects: Array<Projects>) {
    let timeLine = new ProjectTimeLine();
  ...
  public render(): React.ReactElement<IDepartmentProjectProps> {
            ...
            !this.state.isLoading ?
              <div>
                {this.state.projectTimeLine && this.state.projectTimeLine.groups && this.state.projectTimeLine.groups.length > 0 ? (
                  <TimeLineChart groups={this.state.projectTimeLine.groups} items={this.state.projectTimeLine.items} />
                ) : null}
  ...
  private getAllMildStones(projectList: Array<Projects>): void {
  ...
  private getMildStones(projectList: Array<Projects>, allMildStones: Array<MileStones>): void {
    let timeline = new ProjectTimeLine();
  ...
  private getTeamMembersByProject(currentProject: Projects): void {
    sp.web.lists
  ...
  private getKeyDocumentsByProject(currentProject: Projects): void {
    sp.web.lists
  ...
  private getTaggingByProject(currentProject: Projects): void {
